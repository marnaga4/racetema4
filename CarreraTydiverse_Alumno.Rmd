...
author: "Marta Navarro García"
date:  "12-03-2025"  #Pondría la fecha del día actual
params:
  lang: ES
...
```{r}
cran3 <- cran3 %>% mutate(size_md = size * 2^20)
cran3
```
...
```{r}
cran3%>% mutate(size_gb = size_md * 2^10)
```
...
```{r}
cran %>% filter(size <=500000)#0.5 megabytes es equivalente a 500000 bytes
